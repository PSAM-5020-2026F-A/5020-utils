...
def fft(samples, rate):
  _fft_np = np.abs(np.fft.fft(samples * np.hanning(len(samples))))[ :len(samples) // 2]
  _fft = (_fft_np / _fft_np.max()).tolist()
...
def stft(samples, rate, window_len=1024):
  ...
  return _ffts[:len(_freqs), :len(_times)].tolist(), _freqs, _times
